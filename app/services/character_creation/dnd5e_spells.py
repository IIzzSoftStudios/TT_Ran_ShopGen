...
_SCHOOL_HINTS: list[tuple[re.Pattern[str], str]] = [
    (re.compile('\\b(cure|heal|restoration|revivify|resurrection|regenerate)\\b', re.I), 'evocation'),
    (re.compile('\\b(detect|identify|scry|augury|divine|commune|legend lore)\\b', re.I), 'divination'),
    (re.compile('\\b(illusion|mirror|phantasm|invisibility|disguise|silent image)\\b', re.I), 'illusion'),
    (re.compile('\\b(enchant|charm|hold|command|dominate|sleep|confusion)\\b', re.I), 'enchantment'),
    (re.compile('\\b(conjure|summon|teleport|misty|dimension door|gate)\\b', re.I), 'conjuration'),
    (re.compile('\\b(shield|ward|protection|counterspell|dispel|antimagic)\\b', re.I), 'abjuration'),
    (re.compile('\\b(animate dead|blight|contagion|harm|inflict|vampiric)\\b', re.I), 'necromancy'),
    (re.compile('\\b(transmute|polymorph|stone shape|alter self)\\b', re.I), 'transmutation')
